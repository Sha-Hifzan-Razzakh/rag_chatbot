...
from app.agents.tools.registry import ToolRegistry, ToolContext
...
def _normalize_search_results(raw: Any, *, truncate_chars: int) -> List[JsonDict]:
    ...
    # Drop empty ids at end if necessary (still keep ordering)
    return normalized
...
def _extract_answer_and_sources(raw: Any) -> Tuple[str, List[JsonDict]]:
    ...
    if isinstance(raw, dict):
        ...
        # normalize sources if dict-like
    ...
    if isinstance(raw, dict):
        for key in ("results", "items", "documents", "matches", "chunks"):
            if key in raw and isinstance(raw[key], list):
                return raw[key]
        # if dict but unknown, treat as single item
        return [raw]

    # unknown object - treat as single item
    return [raw]
